...
static void *get_hostinfo_fromfd(int fd){
	struct localhost *host;
	for(host = lhost; host != NULL;host = host->next){
		if(host->fd == fd) return host;
	}
...
int listen(int sockfd, int backlog){		//
	coid *hostinfo = get_hostinfo_fromfd(sockfd);
	if(hostinfo == NULL) return -1;
	struct ng_tcp_stream *stream = (struct ng_tcp_stream *)hostinfo;
...
int accept(int sockfd, struct sockaddr *addr, socklen_t *addrlen){		//从全连接队列中取出一个连接
...
static int ng_tcp_handle_syn_established(struct ng_tcp_stream *stream, struct rte_tcp_hdr *tcphdr, int tcplen) {
	if(tcphdr->tcp_flags & RTE_TCP_SYN_FLAG){

	}

	if(tcphdr->tcp_flags & RTE_TCP_PSH_FLAG){		//将收到的数据包立刻返回应用程序 无需等待后续的包
		//发给应用程序处理 -->recv buf
		...
		if(payloadlen > 0){
			...
			rfragment->data = rte_malloc("unsinged char *", payload + 1, 0);
		...
		ackfrag->dport = ntohs(tcphdr->src_port);
		ackfrag->sport = ntohs(tcphdr->dst_port);

		//对端的包id
		//根据包id判断是否重复发送的包
		if(stream->rcv_nxt != ntohs(tcphdr->sent_seq)) {
		...
		uint8_t *payload = (uint8_t *)tcphdr + hdrlen * 4;
		echofrag->data = rte_malloc("unsinged char *", payload + 1, 0);
		if(echofrag->data == NULL){
			rte_free(echofrag);
			return -2;
		}
		memset(echofrag->data, 0,payloadlen + 1);
