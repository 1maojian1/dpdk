...
/*
connfd -> struct localhost
*/
struct localhost{	//每一个udp链接都有一个localhost
	int fd;

	//unsigned int status;	//阻塞非阻塞
	uint32_t localip;
	uint16_t localport;
	uint8_t localmac[RTE_ETHER_ADDR_LEN];
	...
	struct rte_ring *sndbuf;	//每个链接都有sendbuf，recvbuf
	struct rte_ring *rcvbuf;

	struct localhost *prev;		//双向链表
...
static struct localhost *lhost = NULL;	//udp连接列表

/*
	socket() -> connfd -->malloc(localip,localport)
*/
...
static struct localhost *get_hostinfo_fromip_port(uint32_t dip, uint16_t port, int proto){
	struct localhost *host;
	for(host = lhost; host != NULL;host = host->next){
		if(dip == host->localip && port == host->localport && proto == host->protocal) return host;
	}
	return NULL;
}

//udp_process --> recvbuf的数据包
struct offload{
	uint32_t sip;
	uint32_t dip;
...
static int udp_process(struct rte_mbuf *udpmbuf){
	...
	struct localhost *host = get_hostinfo_fromip_port(iphdr->dst_addr, udphdr->dst_port, IPPROTO_UDP);
...
static int nsocket(__attribute__((unused)) int domain, int type, __attribute__((unused)) int protocol){
	...
	if(type == SOCK_DGRAM){
		...
		host->protocal = IPPROTO_UDP;

		host->rcvbuf = rte_ring_create("recv ring", RING_SIZE, rte_socket_id(), RING_F_SP_ENQ | RING_F_SC_DEQ);
		if(host->rcvbuf == NULL){
			rte_free(host);
			return -1;
		}

		host->sndbuf = rte_ring_create("send ring", RING_SIZE, rte_socket_id(), RING_F_SP_ENQ | RING_F_SC_DEQ);
		if(host->sndbuf == NULL){
...
/* 
bind() -> connfd <--> localip:localport
recvfrom() --> connfd <-->localip:localport
*/
static int nbind(int sockfd, const struct sockaddr *addr,
	...
	}else if(host->protocal == IPPROTO_TCP){
...
static int nlisten(int sockfd, int backlog){		//
	void *hostinfo = get_hostinfo_fromfd(sockfd);
	if(hostinfo == NULL) return -1;
	struct ng_tcp_stream *stream = (struct ng_tcp_stream *)hostinfo;
...
static int naccept(int sockfd, struct sockaddr *addr,__attribute__((unused)) socklen_t *addrlen){		//从全连接队列中取出一个连接
	void *hostinfo = get_hostinfo_fromfd(sockfd);
	if(hostinfo == NULL) return -1;
	...
	if(stream->protocol == IPPROTO_TCP){
		...
		while((apt = get_accept_tcb(stream->dport)) == NULL) {				//取的是sockfd
...
static ssize_t nsend(int sockfd, const void *buf, size_t len, int flags){
	//组织一个tcp包
	//将其放到sendbuffer
	void *hostinfo = get_hostinfo_fromfd(sockfd);
	if(hostinfo == NULL) return -1;
	...
	if(stream->protocol == IPPROTO_TCP){
		...
		rte_memcpy(fragment->data, buf, len + 1);
		fragment->length = len;
...
static ssize_t nrecv(int sockfd, void *buf, size_t len, int flags){
	void *hostinfo = get_hostinfo_fromfd(sockfd);
	if(hostinfo == NULL) return -1;
...
static ssize_t nsendto(int sockfd, const void *buf, size_t len, __attribute__((unused)) int flags,
	...
	ol->length = len;
	ol->data = rte_malloc("unsinged char*",len,0);
	if(ol->data == NULL){
		rte_free(ol);
		return -1;
	}
...
int udp_server_entry(__attribute__((unused)) void         *arg) {
	...
	localaddr.sin_addr.s_addr = inet_addr("192.168.5.133"); // 0.0.0.0
...
static struct ng_tcp_stream * ng_tcp_stream_search(uint32_t sip, uint32_t dip, 
								uint16_t sport, uint16_t dport) { // proto
...
static struct ng_tcp_stream * ng_tcp_stream_create(uint32_t sip, uint32_t dip,
						uint16_t sport, uint16_t dport) { // proto
...
static int ng_tcp_handle_syn_rcvd(struct ng_tcp_stream *stream, struct rte_tcp_hdr *tcphdr) {
	if(tcphdr->tcp_flags & RTE_TCP_ACK_FLAG){
		if(stream->status == NG_TCP_STATUS_SYN_RCVD){
			...
			if(listener == NULL){
				rte_exit(EXIT_FAILURE, "ng_tcp_stream_search listener\n");
			}
			//返回给accept，说明有连接
...
static ng_tcp_handle_close_wait(struct ng_tcp_stream *stream, struct rte_tcp_hdr *tcphdr){
	if(tcphdr->tcp_flags & RTE_TCP_FIN_FLAG){
...
static ng_tcp_handle_last_ack(struct ng_tcp_stream *stream, struct rte_tcp_hdr *tcphdr){
	if(tcphdr->tcp_flags & RTE_TCP_ACK_FLAG){
		if(stream->status == NG_TCP_STATUS_LAST_ACK){
			...
			rte_free(stream->sndbuf);
			rte_free(stream->rcvbuf);
...
static int ng_encode_tcp_apppkt(uint8_t *msg, uint32_t sip, uint32_t dip,
	...
	struct rte_tcp_hdr *tcp = (struct rte_tcp_hdr *)(msg + sizeof(struct rte_ether_hdr) +
...
static int ng_tcp_out(struct rte_mempool *mbuf_pool) {
	...
	for (stream = table->tcb_set;stream != NULL;stream = stream->next) {
		...
		if (dstmac == NULL) {
			...
			struct rte_mbuf *tcpbuf = ng_tcp_pkt(mbuf_pool, stream->dip,
											stream->sip, stream->localmac, dstmac, fragment);
...
int main(int argc, char *argv[]) {
	...
	if(ring == NULL){
		rte_exit(EXIT_FAILURE, "ring buffer failed\n");
	}
...
#if ENABLE_MULTHREAD
	//master core发送消息到从线程, 并接收从线程的响应.从线程响应后立即执行pkt_process
